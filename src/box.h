...
public:
	...
	bool overlap(const Box &box) {
		// Holds box2 instance
		Box box2 = box;
		// Box 1 min and max vertices
		Vector3 box1min = Vector3(min());
		Vector3 box1max = Vector3(max());
		// Box 2 min and max vertices
		Vector3 box2min = Vector3(box2.min());
		Vector3 box2max = Vector3(box2.max());
